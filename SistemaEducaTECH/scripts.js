...
function loadData() {
    ...
    if (!storedUsers || !storedAlunos) {
        ...
        alunos = [
            { id: generateUniqueId(), alunoNome: "João Silva", alunoMatricula: "123456", alunoEmail: "joaosilva@unip.br", alunoTurma: turmaExemploId, faltas: 2 },
            { id: generateUniqueId(), alunoNome: "Maria Isabel", alunoMatricula: "123456", alunoEmail: "mariaisabel@unip.br", alunoTurma: turmaExemploId, faltas: 5 }
        ];
...
function openAddAlunoModal() { if (turmas.length === 0) return alert('Cadastre uma turma primeiro.'); openModal('Novo Aluno', [{ id: 'alunoNome', label: 'Nome', required: true }, { id: 'alunoMatricula', label: 'Matrícula (senha)', required: true }, { id: 'alunoEmail', label: 'Email (login)', type: 'email', required: true }, { id: 'alunoFaltas', label: 'Faltas', type: 'number', required: true, min: 0 }, { id: 'alunoTurma', label: 'Turma', type: 'select', options: turmas.map(t => ({ value: t.id, text: t.turmaNome })) }], data => { if (users.some(u => u.email === data.alunoEmail)) return alert('Este email já está em uso.'); const novoAlunoId = generateUniqueId(); alunos.push({ ...data, id: novoAlunoId, faltas: parseInt(data.alunoFaltas) }); users.push({ email: data.alunoEmail, password: data.alunoMatricula, name: data.alunoNome, role: 'aluno' }); saveData(); loadAlunos(); loadStats(); }); }
function loadAlunos() { const tbody = $('alunosTable').querySelector('tbody'); tbody.innerHTML = alunos.map(a => { const turma = turmas.find(t => t.id == a.alunoTurma); return `<tr><td>${a.alunoNome}</td><td>${a.alunoMatricula}</td><td>${a.alunoEmail}</td><td>${turma ? turma.turmaNome : 'N/A'}</td><td>${a.faltas || 0}</td><td><button class="btn-small btn-edit" onclick="editAluno(${a.id})">Editar</button><button class="btn-small btn-delete" onclick="deleteAluno(${a.id})">Excluir</button></td></tr>` }).join('') || '<tr><td colspan="6" class="text-center">Nenhum aluno cadastrado.</td></tr>'; }
...
function gerarRelatorio() {
    ...
    alunos.forEach(aluno => {
        ...
        const media = entregasAluno.length > 0 ? (entregasAluno.reduce((acc, e) => acc + e.nota, 0) / entregasAluno.length).toFixed(2) : 'N/A';
    ...
    const folhasPorRelatorio = 3;
    const relatoriosGerados = 1; // 1 relatório gerado agora
    const folhasEconomizadas = relatoriosGerados * folhasPorRelatorio;
    const arvoresSalvas = folhasEconomizadas / 10000; // <-- Correção: 1 árvore a cada 10.000 folhas

    // Texto do alerta atualizado e com 4 casas decimais para mostrar 0.0003
    alert(`🌳\n\Com seus relatórios digitais, você poupou ${folhasEconomizadas} folhas e contribuições para salvar ${arvoresSalvas.toFixed(4)} árvores. Vamos juntos proteger o planeta! 🌍\"`);
} 
